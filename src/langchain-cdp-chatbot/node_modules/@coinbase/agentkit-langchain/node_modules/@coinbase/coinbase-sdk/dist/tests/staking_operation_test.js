...
describe("StakingOperation", () => {
    ...
    it("should initialize a new StakingOperation", () => {
        const op = new staking_operation_1.StakingOperation(utils_1.VALID_STAKING_OPERATION_MODEL);
    ...
    it("should raise an error when initialized with a model of a different type", () => {
        expect(() => {
            new staking_operation_1.StakingOperation(null);
        }).toThrow(Error);
    });
    ...
    describe(".getTransactions", () => {
        it("return the the array of transactions", () => {
            ...
            expect(op.getStatus()).toEqual(client_1.StakingOperationStatusEnum.Initialized);
        });
    });
    describe(".getSignedVoluntaryExitMessages", () => {
        it("return the the array of signed exit messages", () => {
            const op = new staking_operation_1.StakingOperation(utils_1.VALID_NATIVE_ETH_UNSTAKE_OPERATION_MODEL);
    ...
    describe("StakingOperation.fetch", () => {
        ...
        it("should fetch staking operation without walletId", async () => {
            coinbase_1.Coinbase.apiClients.stake.getExternalStakingOperation = jest.fn().mockResolvedValue({
                data: mockStakingOperationModel,
            });
            const stakingOperation = await staking_operation_1.StakingOperation.fetch(networkId, addressId, stakingOperationId);
            expect(coinbase_1.Coinbase.apiClients.stake.getExternalStakingOperation).toHaveBeenCalledWith(networkId, addressId, stakingOperationId);
            expect(stakingOperation.getID()).toBe(stakingOperationId);
            expect(stakingOperation.getStatus()).toBe(client_1.StakingOperationStatusEnum.Initialized);
        ...
        it("should fetch staking operation with walletId", async () => {
            coinbase_1.Coinbase.apiClients.walletStake.getStakingOperation = jest.fn().mockResolvedValue({
                data: mockStakingOperationModel,
            });
            const stakingOperation = await staking_operation_1.StakingOperation.fetch(networkId, addressId, stakingOperationId, walletId);
            expect(coinbase_1.Coinbase.apiClients.walletStake.getStakingOperation).toHaveBeenCalledWith(walletId, addressId, stakingOperationId);
            expect(stakingOperation.getID()).toBe(stakingOperationId);
            expect(stakingOperation.getStatus()).toBe(client_1.StakingOperationStatusEnum.Initialized);
            expect(stakingOperation.getNetworkID()).toBe(networkId);
            expect(stakingOperation.getAddressID()).toBe(addressId);
        ...
        it("should throw an error if walletId is empty", async () => {
            await expect(staking_operation_1.StakingOperation.fetch(networkId, addressId, stakingOperationId, "")).rejects.toThrow("Invalid wallet ID");
        });
    });
    describe(".wait", () => {
        it("should return the non-terminal StakeOperation state when the stake operation is not complete", async () => {
            const stakingOperation = new staking_operation_1.StakingOperation(utils_1.VALID_STAKING_OPERATION_MODEL);
            coinbase_1.Coinbase.apiClients.stake.getExternalStakingOperation = (0, utils_1.mockReturnValue)((0, utils_1.mockStakingOperation)(client_1.StakingOperationStatusEnum.Complete));
            await stakingOperation.wait();
            expect(stakingOperation.getStatus()).toBe(client_1.StakingOperationStatusEnum.Complete);
        });
        it("should raise a timeout error", async () => {
            const stakingOperation = new staking_operation_1.StakingOperation(utils_1.VALID_STAKING_OPERATION_MODEL);
            coinbase_1.Coinbase.apiClients.stake.getExternalStakingOperation = (0, utils_1.mockReturnValue)(stakingOperation);
            await expect(stakingOperation.wait({
                intervalSeconds: 0.2,
        ...
        it("should raise a timeout when the request takes longer than the timeout", async () => {
            const stakingOperation = new staking_operation_1.StakingOperation(utils_1.VALID_STAKING_OPERATION_MODEL);
            coinbase_1.Coinbase.apiClients.stake.getExternalStakingOperation = (0, utils_1.mockReturnValue)(new Promise(resolve => {
                setTimeout(() => {
        ...
        it("all getters should work", async () => {
            ...
            expect(stakingOperation.getStatus()).toBe(client_1.StakingOperationStatusEnum.Initialized);
            expect(stakingOperation.getNetworkID()).toBe(coinbase_1.Coinbase.networks.EthereumHolesky);
    ...
    describe("StakingOperation.loadTransactionsFromModel", () => {
        it("should not add duplicate transactions", () => {
            ...
            const modelWithInitialTransactions = {
                ...utils_1.VALID_STAKING_OPERATION_MODEL,
                transactions: [
                    {
            ...
            const op = new staking_operation_1.StakingOperation(modelWithInitialTransactions);
