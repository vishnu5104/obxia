...
describe("Trade", () => {
    let fromKey, networkId, walletId, addressId, fromAmount, toAmount, ethAmount, usdcAmount, tradeId, unsignedPayload, ethAsset, usdcAsset, transactionModel, approveTransactionModel, fromAsset, toAsset, model, tradesApi, trade;
    beforeEach(() => {
        fromKey = ethers_1.ethers.Wallet.createRandom();
        networkId = "base-sepolia";
        walletId = (0, crypto_1.randomUUID)();
        addressId = fromKey.address;
        fromAmount = new decimal_js_1.Decimal(100);
        toAmount = new decimal_js_1.Decimal(100000);
        ethAsset = { network_id: networkId, asset_id: "eth", decimals: 18 };
        usdcAsset = { network_id: networkId, asset_id: "usdc", decimals: 6 };
        ethAmount = fromAmount.div(new decimal_js_1.Decimal(Math.pow(10, ethAsset.decimals)));
        usdcAmount = toAmount.div(new decimal_js_1.Decimal(Math.pow(10, usdcAsset.decimals)));
        tradeId = ethers_1.ethers.Wallet.createRandom().address;
        unsignedPayload =
        ...
        coinbase_1.Coinbase.apiClients.trade = tradesApi;
        trade = new trade_1.Trade(model);
    });
    describe("constructor", () => {
        ...
        it("should raise an error when initialized with a model of a different type", () => {
            expect(() => new trade_1.Trade(null)).toThrow("Trade model cannot be empty");
        });
    });
    describe("#getId", () => {
    ...
    describe("#getFromAmount", () => {
        ...
        it("should return the from amount in the whole units when the from asset is something else", () => {
            model = {
                ...
                from_amount: new decimal_js_1.Decimal(500000),
            };
            coinbase_1.Coinbase.apiClients.trade.getTrade = (0, utils_1.mockReturnValue)(model);
            const trade = new trade_1.Trade(model);
            expect(trade.getFromAmount()).toEqual(new decimal_js_1.Decimal(500));
        });
    });
    ...
    describe("#getToAmount", () => {
        ...
        it("should return the to amount in the whole units when the to asset is something else", () => {
            model = {
                ...
                to_amount: new decimal_js_1.Decimal(42000000),
            };
            coinbase_1.Coinbase.apiClients.trade.getTrade = (0, utils_1.mockReturnValue)(model);
            const trade = new trade_1.Trade(model);
            expect(trade.getToAmount()).toEqual(new decimal_js_1.Decimal(42));
        });
    });
    ...
    describe("#getTransaction", () => {
        it("should return the Transaction", () => {
            expect(trade.getTransaction()).toBeInstanceOf(transaction_1.Transaction);
        });
    ...
    describe("#getStatus", () => {
        it("should returns the transaction status", () => {
            expect(trade.getStatus()).toBe(types_1.TransactionStatus.PENDING);
        });
    });
    describe("#sign", () => {
    ...
    describe("#broadcast!", () => {
        let broadcastedTradeModel, broadcastedTransactionModel, broadcastedApproveTransactionModel, signedPayload;
        beforeEach(async () => {
            signedPayload =
        ...
        it("broadcasts the trade with the signed tx payload", async () => {
            ...
            expect(coinbase_1.Coinbase.apiClients.trade.broadcastTrade).toHaveBeenCalledWith(walletId, addressId, tradeId, {
        ...
        it("returns the broadcasted trade", async () => {
            ...
            expect(broadcastedTrade).toBeInstanceOf(trade_1.Trade);
            expect(broadcastedTrade).toBe(trade);
            expect(trade.getStatus()).toBe(types_1.TransactionStatus.BROADCAST);
        });
        describe("when the transaction is not signed", () => {
            ...
            it("raises a not signed error", async () => {
                expect(trade.broadcast()).rejects.toThrow(errors_1.NotSignedError);
            });
        });
    ...
    describe("#reload", () => {
        ...
        it("should update the trade transaction", async () => {
            await trade.reload();
            expect(trade.getTransaction().getStatus()).toBe(types_1.TransactionStatus.COMPLETE);
            expect(trade.getToAmount()).toEqual(new decimal_js_1.Decimal(500000000).div(new decimal_js_1.Decimal(Math.pow(10, usdcAsset.decimals))));
        });
        it("should update properties on the trade", async () => {
            expect(trade.getToAmount()).toEqual(usdcAmount);
            await trade.reload();
            expect(trade.getToAmount()).toEqual(new decimal_js_1.Decimal(updatedModel.to_amount).div(new decimal_js_1.Decimal(Math.pow(10, usdcAsset.decimals))));
        });
    });
    describe("#wait", () => {
        it("should return the completed Trade when the trade is completed", async () => {
            ...
            const updatedModel = { ...model, transaction: updatedTransactionModel };
            tradesApi.getTrade
                .mockResolvedValueOnce({ data: model })
                .mockResolvedValueOnce({ data: updatedModel });
            const trade = new trade_1.Trade(model);
            await trade.wait();
            expect(trade.getStatus()).toBe(types_1.TransactionStatus.COMPLETE);
        });
        it("should return the failed Trade", async () => {
            ...
            const trade = new trade_1.Trade(model);
            await trade.wait();
            expect(trade.getStatus()).toBe(types_1.TransactionStatus.FAILED);
        });
        it("should raise a timeout error", async () => {
            await expect(trade.wait({
                intervalSeconds: 0.2,
        ...
        it("should raise a timeout when the request takes longer than the timeout", async () => {
            tradesApi.getTrade.mockResolvedValueOnce(new Promise(resolve => {
                setTimeout(() => {
                    resolve({ data: model });
                }, 400);
            }));
    ...
    describe("#toString", () => {
        it("should return the same value as toString", () => {
            expect(trade.toString()).toEqual(`Trade { transfer_id: '${trade.getId()}', network_id: '${trade.getNetworkId()}', address_id: '${trade.getAddressId()}', from_asset_id: '${trade.getFromAssetId()}', to_asset_id: '${trade.getToAssetId()}', from_amount: '${trade.getFromAmount()}', to_amount: '${trade.getToAmount()}', status: '${trade.getStatus()}' }`);
        });
    });
});
