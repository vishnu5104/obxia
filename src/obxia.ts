...
export let k: obxiaCtx;
export let globalOpt: obxiaOpt;
...
/**
 * Initialize obxia context. The starting point of all obxia games.
 *
 * @example
 * ```js
 * // Start obxia with default options (will create a fullscreen canvas under <body>)
 * obxia()
 *
 ...
 * })
 *
 * // All obxia functions are imported to global after calling obxia()
 * add()
 * onUpdate()
 * onKeyPress()
 * vec2()
 *
 * // If you want to prevent obxia from importing all functions to global and use a context handle for all obxia functions
 * const k = obxia({ global: false })
 *
 * k.add(...)
 * k.onUpdate(...)
 * k.onKeyPress(...)
...
>(
    gopt: obxiaOpt<TPlugins, TButtons> = {},
): TPlugins extends [undefined] ? obxiaCtx<TButtons, TButtonsName>
    : obxiaCtx<TButtons, TButtonsName> & MergePlugins<TPlugins> =>
{
    globalOpt = gopt;
    ...
    const plugins = gopt.plugins as obxiaPlugin<Record<string, unknown>>[];
    ...
    if (gopt.global !== false) {
        for (const key in k) {
            (<any> window[<any> key]) = k[key as keyof obxiaCtx];
        }
    }
    ...
    return k as TPlugins extends [undefined] ? obxiaCtx<TButtons, TButtonsName>
        : obxiaCtx<TButtons, TButtonsName> & MergePlugins<TPlugins>;
};
