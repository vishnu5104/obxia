...
```sh
git clone https://github.com/obxiajs/obxia.git # to clone the repo.
cd obxia # to enter the project directory.
...
1. The source entry point is `src/obxia.ts`, editing any files referenced will
...
obxia API documentation is auto-generated by JSDoc comments in the source code.
...
on `types.ts` file, in the `obxiaCtx` interface, this is because what
`obxia()` returns is a `obxiaCtx` object. So even if component files has the
