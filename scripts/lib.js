...
const fmts = (name) => [
  {
    ...
    footer: {
      js: 'window.kaboom = obxia.default; window.obxia = obxia.default',
    },
  },
...
export async function build() {
  return Promise.all(
    [fmts('obxia'), fmts('kaboom')].flat().map((fmt) => {
      return esbuild
        .build({
          ...config,
          ...fmt,
    ...
    plugins: [
      {
        ...
        setup(b) {
          b.onEnd(() => {
            console.log(`-> ${fmts('obxia')[0].outfile}`);
          });
        },
      },
    ],
...
export async function genDTS() {
  ...
  // generate global decls for obxiaCtx members
  let globalDts = '';

  globalDts +=
    'import { obxiaCtx } from "./types"\nimport { obxia as obxia } from "./obxia"\n';
  globalDts += 'declare global {\n';

  for (const stmt of stmts) {
    if (stmt.name === 'obxiaCtx') {
      if (stmt.kind !== 'InterfaceDeclaration') {
        throw new Error('obxiaCtx must be an interface.');
      }
      for (const name in stmt.members) {
        globalDts += `\tconst ${name}: obxiaCtx["${name}"]\n`;
      }
      globalGenerated = true;
    }
  }

  globalDts += `\tconst obxia: typeof obxia\n`;
  globalDts += `\tconst kaboom: typeof obxia\n`;

  globalDts += '}\n';

  if (!globalGenerated) {
    throw new Error('obxiaCtx not found, failed to generate global defs.');
  }
...
export function serve(opt = {}) {
  ...
  app.get('/:name', async (req, res) => {
    ...
    <body>
    <script src="/dist/obxia.js"></script>
