...
- added the `animate()` component to _animate_ the properties of an object using
  keyframes. Check out
  [Animation Example](https://play.obxiajs.com/?example=animation)
...
- (**! break**) removed compatibilty to use two obxia frames in the same page,
...
- added `obxiaOpt.debugKey` for customizing the key used to toggle debug mode.
...
- deprecated `kaboom()` in favor of `obxia()` (you can still use `kaboom*`)
