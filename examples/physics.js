...
const trajectoryText = add([pos(20, 20), text(`0`)]);

function ballistics(pos, vel, t) {
  return pos
    .add(vel.scale(t))
    .add(vec2(0, 1).scale(getGravity() * t * t * 0.5));
}
  ...
  const bean = add([
    ...
    offscreen({ destroy: true }),
    {
      ...
      destroy() {
        const a =
          results
            .map((d) => Math.sqrt((d[1] - d[2]) * (d[1] - d[2])))
...
const heightText = add([pos(100, heightGoal), text(`0%`)]);

onUpdate(() => {
  heightText.text = `${(
    (100 * (heightResult - heightGoal)) /
    heightGoal
  ).toFixed(2)}%`;
});
