...
add([pos(80, 80), circle(40), color(BLUE), area()]);

add([pos(180, 210), circle(20), color(BLUE), area()]);

add([pos(40, 180), rect(20, 40), color(BLUE), area()]);

add([pos(140, 130), rect(60, 50), color(BLUE), area()]);
...
onUpdate(() => {
  ...
  shapes.forEach((s1) => {
    if (
      shapes.some((s2) => s1 !== s2 && s1.getShape().collides(s2.getShape()))
    ) {
      s1.color = RED;
    } else {
      s1.color = BLUE;
